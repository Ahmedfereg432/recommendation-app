...
try:
    ...
    logger.error("Error: 'GRAD.products.csv' file not found. Please ensure the file exists in the correct directory.")
    sys.exit(1)
except pd.errors.ParserError:
    logger.error("Error: Failed to parse 'GRAD.products.csv'. Ensure the file is a valid CSV.")
    sys.exit(1)
except Exception as e:
    logger.error(f"Unexpected error while loading 'GRAD.products.csv': {e}")
    sys.exit(1)
...
def get_content_based_recommendations(product_id, cosine_sim=cosine_sim, products_df=products_df, top_n=20):
    ...
    try:
        ...
        if len(idx) == 0:
            logger.warning(f"Product ID {product_id} not found in products_df.")
            return pd.DataFrame()
        idx = idx[0]
        ...
        logger.error(f"Error occurred while getting content-based recommendations for product {product_id}.")
        return pd.DataFrame()
    except Exception as e:
        logger.error(f"Unexpected error in content-based recommendations: {e}")
        return pd.DataFrame()
...
def get_collaborative_recommendations(user_id, model=svd, products_df=products_df, top_n=10):
    ...
    if model is None or interactions_df.empty:
        logger.warning("Collaborative Filtering not available due to lack of model or interaction data.")
        return pd.DataFrame()
...
def get_hybrid_recommendations(user_id, product_id=None, top_n=50, collab_weight=0.4, interactions_df=interactions_df):
    """
    ...
    if product_id is not None:
        ...
        logger.info(f"Using provided product ID '{product_id}' for content-based filtering.")
    else:
        if not user_interactions.empty:
            base_product_ids = user_interactions['product_id'].unique().tolist()
            logger.info(f"Using user's interaction products {base_product_ids} for content-based filtering.")
        else:
            merged_df = interactions_df.merge(products_df[['product_id', 'categories']], on='product_id', how='left')
            interacted_categories = merged_df['categories'].value_counts().index[0] if not merged_df.empty else 'Electronics'
            popular_products = products_df[products_df['categories'] == interacted_categories].sort_values(by='rate', ascending=False).head(3)
            base_product_ids = popular_products['product_id'].tolist()
            logger.info(f"User has no interactions. Using popular products from category '{interacted_categories}'.")
    ...
    docs = {
        ...
        "endpoints": {
            "/hybrid": {
                ...
                "example": "/hybrid?user_id=68476733e0b3981f8a711165&top_n=5"
            },
            "/content": {
                "method": "GET",
...
def hybrid_recommendations():
    try:
        ...
        recommendations = get_hybrid_recommendations(user_id=user_id, product_id=product_id, top_n=top_n)
        ...
        logger.error(f"Error in hybrid_recommendations: {e}")
...
def content_recommendations():
    try:
        ...
        logger.error(f"Error in content_recommendations: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500
...
if __name__ == '__main__':
    ...
    print(f"- Hybrid: http://localhost:{port}/hybrid?user_id=68476733e0b3981f8a711165")
    print(f"- Content: http://localhost:{port}/content?product_id=683dc98cc029c78e3af4fb0d")
    print(f"{'='*50}\n")
